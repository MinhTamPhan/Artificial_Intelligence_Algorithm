...
struct ToaDo
{
	...
	friend float tinhKC(const ToaDo& td1, const ToaDo& td2)
	{
		return sqrt(pow(td1.kinhDo - td2.kinhDo, 2) + pow(td1.viDo - td2.viDo,2));
	}
};
...
class CMyGraph
{
	...
	static vector<ToaDo> H;
public:
	static map<int, string>nStreet;
	void readInput(int &start,int &end,char *filename2);
	
	float runGreedy(int start,int end,vector<int>&previous);
